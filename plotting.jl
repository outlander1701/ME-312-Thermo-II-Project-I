...
function η_th_vs_rp(cycle_func, r_p, Gasses, T_1, T_3, η_c, η_t, ϵ, ribbon, vary_label)
    N = length(r_p)
    M = length(Gasses)
    ...
    for i ∈ 1:M
        ...
        elseif (ribbon == true)
            name = Gasses[i].name
            plot!(r_p, η_th_80, fillrange=η_th_100, fillalpha = 0.5, c = i, alpha=0.2, label=name, gradient=true)
            plot!(r_p, η_th_80, label=nothing, line=(1, :dash), c=i)
            plot!(r_p, η_th_100, label=nothing, c=i)
...
function η_II_vs_rp(cycle_func, r_p, Gasses, T_min, T_max, η_c, η_t, ϵ, ribbon, vary_label)
    ...
    for i ∈ 1:M
        ...
        for j ∈ 1:N
            η_II[j] = cycle_func(r_p[j], T_min, T_max, η_c, η_t, ϵ, Gasses[i])
        end

        η_II_80 = Array{Float64, 1}(undef, N)
        for j ∈ 1:N
            η_II_80[j] = cycle_func(r_p[j], T_min, T_max, η_c, η_t, 0.8, Gasses[i])
        end

        η_II_100 = Array{Float64, 1}(undef, N)
        for j ∈ 1:N
            η_II_100[j] = cycle_func(r_p[j], T_min, T_max, η_c, η_t, 1, Gasses[i])
        end
...
function work_out_net_vs_rp(cycle_func, r_p, T_1, T_3, η_c, η_t, Gasses, ribbon)
    ...
    for i ∈ 1:M
        ...
        if ribbon == true
            name = Gasses[i].name
            plot!(r_p, w_out_net_80, fillrange=w_out_net_100, fillalpha = 0.5, c = i, alpha=0.2, label=latexstring("$name: \$0.8 < \\eta_{C,T} < 1\$"), gradient=true)
            plot!(r_p, w_out_net_80, label=nothing, line=(1, :dash), c=i)
            plot!(r_p, w_out_net_100, label=nothing, c=i)
        end
    end
